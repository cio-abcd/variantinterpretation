...
parser.add
parser.add_argument("-v", "vembrane_table", required=True, type=Path)
parser.add_argument("-r", "refseq_list", required=True, type=Path)
parser.add_argument("-D", "variant_DBi", required=True, type=Path)
parser.add_argument("-o", "--outfile", required=True, type=Path)
parser.add_argument("-rv", "--removed_variants", required=True, type=Path)
...
# Script used
print("Script: WXS_process_variants_report.py")

vembrane_table = pd.read_csv(args.vembrane_table, sep="\t",low_memory=False)

# Remove INTERGENIC_VARIANTS
vs_intergenic = []
vs_report = []

for ir_index, csq_consequence in enumerate(vembrane_table["CSQ_Consequence"]):
    if csq_consequence == "intergenic_variant":
        vs_intergenic.append(ir_index)
    else:
        vs_report.append(ir_index)

#vs_filter_intergenic = vembrane_table[vembrane_table["CSQ_Consequence"]=="intergenic_variant"]
#data_report = vembrane_table[vembrane_table["CSQ_Consequence"]!="intergenic_variant"]

# Report variants
data_report = vembrane_table.loc[vs_report, :]

# Removed variants
intergenic_variants = vembrane_table.loc[vs_intergenic, :]
