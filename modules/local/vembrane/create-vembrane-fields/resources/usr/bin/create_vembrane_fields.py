...
def commastring(vstr, sep=','):
    '''
    Converts a string of comma separated values to iterable strings
    '''
    values = [v.strip() for v in vstr.split(",")]
    return values
...
def parse_arguments():
    ...
    parser.add_argument(
        "--csq_fields",
        help="comma-separated or space-separated CSQ fields. The fields only allow letters, numbers, underscores, square brackets and mathematical operands.",
        type=commastring,
        nargs="+",
    )
    parser.add_argument(
        "--format_fields",
        help="comma-separated or space-separated FORMAT fields. The fields only allow letters, numbers, underscores, square brackets and mathematical operands.",
        type=commastring,
        nargs="+",
    )
    parser.add_argument(
        "--info_fields",
        help="comma-separated or space-separated INFO fields. The fields only allow letters, numbers, underscores, square brackets and mathematical operands.",
        type=commastring,
        nargs="+",
    )
    parser.add_argument(
...
def input_check(input_strings):
    for input_string in input_strings:
        if re.fullmatch(r"^[A-Za-z0-9+\-*/\[\]_,]+$", input_string) is None:
            raise ValueError(
                f'Error: The input_field "{input_string}" should only contain letters, numbers, square brackets, mathematical operands or underscores. Separate entries are separated by commas.'
            )
...
if __name__ == "__main__":
    ...
    if args.format_fields:
        ...
        args.format_fields = flatten(args.format_fields) if args.format_fields else []
        # Formatting the FORMAT fields.
        print(args.format_fields)
    ...
    if args.info_fields:
        # First flatten list of lists generated by splitting comma-separated values
        args.info_fields = flatten(args.info_fields) if args.info_fields else []
    ...
    if args.csq_fields:
        # First flatten list of lists generated by splitting comma-separated values
        args.csq_fields = flatten(args.csq_fields) if args.csq_fields else []
