...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Formats fields (i.e. words) from a string for vembrane input. It outputs the format: column_name["FIELD"][sampleindex]. This script helps to convert input parameters into vembrane language. You can also perform calculation operations which will be transcribed to vembrane-compatible fields, e.g. , AD[0]/DP will be converted to column_name["AD"][sampleindex][0]/column_name["DP"][sampleindex].'
    )
    parser.add_argument(
        "input_string", help="Input string in which fields are defined as linked letters disrupted by non-letters."
    )
    parser.add_argument(
        "--column_name",
        help="Can be either FORMAT, INFO or CSQ (or another INFO field annotation supported by vembrane.)",
    )
    parser.add_argument(
        "--fields_with_sampleindex",
        nargs="?",
        const="all",
        help="Fields to apply sampleindex parameter (options: None, all, [fields])",
    )
    parser.add_argument("--sampleindex", type=int, help="Sample index for all fields_with_sampleindex.")
    parser.add_argument(
        "--header_prefix",
        default=None,
        help="If enabled, instead of formatting as describes only outputs defined prefix to the string.",
    )
    ...
    try:
        output_string = format_string(
            args.input_string, args.column_name, args.fields_with_sampleindex, args.sampleindex, args.header_prefix
        )
